...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		for i in range(m):
			data = X[i, :]
			min = np.argsort(np.linalg.norm((data - clusters), axis=1)).item(0)
			label[i] = min
		for index in range(k):
			points = np.where(label == index)[0]
			if len(points) > 0:
				clusters[index, :] = X[points].mean(axis=0)
...
if __name__ == '__main__':
	...
	plt.plot(opt_k, min(cost_k_list))
	...
	center_plot, = plt.plot(X_cluster[:, 0], X_cluster[:, 1],"bo")
	"*** END YOUR CODE HERE ***"
