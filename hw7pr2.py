...
def find_cost(X, y, W , reg):
	...
	"*** YOUR CODE HERE ***"
	cost = (np.sum((y- X @ W) **2)+ reg*np.abs(W).sum())/len(y)
...
def grad_lasso(
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		W_grad = find_grad(X[index], y[index], W, reg)
		W = prox(W - lr * W_grad, reg * lr)
		cost = find_cost(X[index], y[index], W, reg)
...
if __name__ == '__main__':
	...
	features_name = np.array(df.columns)
	top_features = features_name[np.argsort(-W[:, 0])[:5]+1]
	"""*** END YOUR CODE HERE ***"""
	print(top_features)
