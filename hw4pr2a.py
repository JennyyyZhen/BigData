...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	log = y * np.log(sigmoid(X @ W)) + (1-y) * np.log(1-sigmoid(X @ W))
	nll = -log.sum() + reg/2*np.linalg.norm(W)**2
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
	reg_list = [0., 0.01, 0.1, 0.5, 1.0, 5.0, 10.0, 50.0]
	for index in range(len(reg_list)):
		reg = reg_list[index]
		W_opt, obj = grad_descent(X_train, y_train, reg, lr = 2e-4, print_freq = 100)
