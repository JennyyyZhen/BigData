...
def linreg(X, y, reg=0.0):
	...
	eye[0, 0] = 0 
	W_opt = np.linalg.solve(np.matmul(X.T, X )+ reg * eye,np.matmul(X.T , y))
	"*** END YOUR CODE HERE ***"
...
def find_RMSE(W, X, y):
	...
	'''
	"*** YOUR CODE HERE ***"
	MSE = np.linalg.norm(y-predict(W,X), 2) ** 2 / X.shape[0]
	"*** END YOUR CODE HERE ***"
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	for i in range(len(reg_list)):
		W_list.append(linreg(X_train, y_train, reg = reg_list[i]))
...
def linreg_no_bias(X, y, reg = 0.0):
	...
	m = X.shape[0]
	ones = np.eye(m)
	equation = X.T @ (np.eye(m) - np.ones(m) / m)
	W_opt = np.linalg.solve(equation @ X + reg * np.eye(equation.shape[0]), \
		equation @ y)
...
def grad_descent(X_train, y_train, X_val, y_val, reg = 0.0, \
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps and iter_num < max_iter:
		...
		"*** YOUR CODE HERE ***"
		# calculate norms
		rmse_train = np.sqrt(np.linalg.norm((X_train @ W).reshape((-1, 1)) + b - y_train) ** 2 / m_train)
		rmse_val = np.sqrt(np.linalg.norm((X_val @ W).reshape((-1, 1)) + b - y_val) ** 2 / m_val)
		# append values
		obj_train.append(rmse_train)
		obj_val.append(rmse_val)
		# calculate gradient
